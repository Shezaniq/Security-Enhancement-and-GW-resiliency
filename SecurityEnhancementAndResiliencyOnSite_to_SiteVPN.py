...
sheet1_PROTGRP = pd.read_csv("C:/MISC/BAU/Python Script/PROTGRP/sheet1_PROTGRP.csv")
sheet1_IPRT = pd.read_csv("C:/MISC/BAU/Python Script/IPRT/sheet1_IPRT.csv")
...
# Check if "IKEP-1" is present in the $dn column          
        ...
        for i_protprg, row_PROTGRP in sheet1_PROTGRP.iterrows():
            dn_substring_protgrp = sheet1_PROTGRP.iloc[i_protprg,0][:54]
            
            if dn_substring_protgrp == substring_IKEP:
                row_PROTGRP = row_PROTGRP.drop(row_PROTGRP.index)
                
              
                #modified_data_PROTGRP.append(row_PROTGRP)
                break
            elif dn_substring_protgrp != substring_IKEP:
                
                row_PROTGRP["$dn"] = sheet1.iloc[i,0][:54]+"IKEPROTGRP-1"
                row_PROTGRP["ikePDN"] = sheet1.iloc[i,0][10:54]+"IKE-1"
                row_PROTGRP["role"] = 0
                row_PROTGRP["switchTimeMax"] = 240
                row_PROTGRP["switchTimeMin"] = 120
                row_PROTGRP["switchBackMode"] = 1
                row_PROTGRP["ikePDN.1"] = row_PROTGRP["$dn"][10:54]+"IKE-3"
                row_PROTGRP["role.1"] = 1
               
        modified_data_PROTGRP.append(row_PROTGRP)    
        
        for i_iprt, row_IPRT in sheet1_IPRT.iterrows():
            if "IPRT-1" in row_IPRT["$dn"]:
                 
                row_IPRT["destIpAddr"] = "10.138.7.0"
                row_IPRT["destinationIpPrefixLength"] = 24
                break
            #modified_data_IPRT.append(row_IPRT)    
        modified_data_IPRT.append(row_IPRT) 
...
# Convert the modified_data_PROTGRP list to a DataFrame
modified_sheet_PROTGRP = pd.DataFrame(modified_data_PROTGRP)

# Write the modified data to a new csv file
modified_sheet_PROTGRP.to_csv("C:/MISC/BAU/Python Script/PROTGRP/sheet3_modified_PROTGRP.csv", index=False)  

# Convert the modified_data_IPRT list to a DataFrame
modified_sheet_IPRT = pd.DataFrame(modified_data_IPRT)

# Write the modified data to a new csv file
modified_sheet_IPRT.to_csv("C:/MISC/BAU/Python Script/IPRT/sheet3_modified_IPRT.csv", index=False) 
